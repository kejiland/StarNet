...
| API 请求地址 | OpenAI 兼容的接口地址（中转站填根地址即可，自动补全 /v1） | `https://api.baipiao.eu.org` |
...
> **关于中转站 / 代理站（重要）**：
> - 请求地址填**根地址**即可（如 `https://api.baipiao.eu.org`），程序会自动尝试 `/v1/chat/completions` 和 `/chat/completions` 等路径，无需手动加 `/v1`；
